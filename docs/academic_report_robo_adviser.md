...
Reconciliation is the quality gate of the build. The implemented harness compares computed values at an absolute tolerance of \(10^{-6}\), with a three-valued status per check: PASS when an Excel reference exists and the absolute deviation is within tolerance; FAIL when an Excel reference exists and the deviation exceeds tolerance; and SKIP when no Excel reference exists for that check. The three-valued status is a deliberate methodological choice. An earlier iteration of the harness reported PASS in the absence of an Excel reference, on the grounds that the Python computation had passed its own internal consistency checks. This was misleading: a reader of the report seeing "18 of 18 checks passed" could reasonably infer that Excel parity had been verified when in fact only Python self-consistency had been exercised. The SKIP status corrects this by rendering the absence of an Excel reference as a visible, counted gap rather than a silent affirmative.

At the current state of the project, the reconciliation report produces 18 PASS results against 14 SKIP results over 32 total checks. The 18 PASSes cover Python-side self-consistency: each computed portfolio's weights sum to unity within floating-point tolerance, each covariance matrix is positive semi-definite, each frontier is monotonic, and so on. The 14 SKIPs enumerate every point at which the Excel audit model must eventually supply a reference CSV for true independent validation — three in Phase 1 (expected-return vector, covariance matrix, GMVP weights), five in Phase 2 (optimal weights at risk-aversion values \(A \in \{0.5, 2.0, 3.5, 6.0, 10.0\}\)), one in Phase 3 (the long-only 100-point frontier), and five in Phase 3b (the short-allowed GMVP, both tangencies, the short-allowed 100-point frontier, and equal-weight statistics). Each SKIP row names the exact CSV filename the harness expects, so that when the Excel model is complete and its CSV exports are placed in `data/reconciliation/`, the corresponding row flips from SKIP to PASS or FAIL on the next harness run without any code change.

Tangency rows additionally carry a `solver_path` field recording whether the primary or fallback SLSQP path (§2.6) produced the reported solution. This provenance supports the methodological claim that on the implemented dataset the primary path is never optimal under the deployed bound regimes, and allows future debugging to distinguish between tangency divergences caused by bound-regime differences versus solver-path differences.

Failure taxonomy for FAIL results, anticipated once Excel reconciliation is active: data-pipeline errors (date-range misalignment, annualization-factor mismatch, log-versus-simple-return inconsistency); matrix algebra mismatches (transposition errors, index misalignment between named ranges and asset ordering); optimizer-tolerance issues (SLSQP `ftol` set below the reconciliation tolerance, producing spurious FAILs that resolve with tolerance tightening); and annualization mistakes (monthly data scaled by 252 instead of 12, or vice versa). Each category suggests a targeted investigation path. In the absence of active FAILs at the time of this report, the taxonomy is preparatory: it documents the debugging framework that will apply when Excel parity is exercised.
