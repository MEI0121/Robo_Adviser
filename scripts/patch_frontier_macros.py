...
FRONTIER_MACRO_BLOCK: list[str] = [
    ...
    "    '",
    "    ' REVISED 2026-04 with three fixes:",
    ...
    "    '   (3) Keep Application.Calculation=Automatic during the Solver",
    "    '       loop. Setting it to Manual (as the previous version did)",
    "    '       prevents Solver from evaluating formulas in retCell /",
    "    '       varCell / sumCell during its iterations, which returns",
    "    '       status 5 (no feasible solution) every row.",
    "",
    ...
    "    ' NOTE: intentionally leave Calculation=Automatic. Setting it to",
    "    '   xlCalculationManual here blocks Solver from seeing formula",
    "    '   updates when it changes weights during its iterations, which",
    "    '   makes retCell / varCell / sumCell stale and causes Solver to",
    "    '   return status 5 (no feasible solution) on every row.",
    "",
    "    For i = 4 To 103",
...
FRONTIER_SHORT_MACRO_BLOCK: list[str] = [
    ...
    "    '",
    "    ' REVISED 2026-04 with three fixes (identical to GenerateFrontier):",
    ...
    "    '   (3) Calculation stays Automatic during the Solver loop.",
    "",
    "    Dim ws As Worksheet, wsOpt As Worksheet",
    "    Dim i As Long, statusCode As Integer",
    "    Dim weightRange As String",
